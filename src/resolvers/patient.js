...
export default {
  ...
  Mutation: {
    createPatient: async (root, args, { req }, info) => {
      ...
      let historyId;
      try{
        historyId = await Counters.findOneAndUpdate({ _id: 'historyId'}, { $inc: { sequence_value: 1 } }, {new: true })
    ...
    updatePatient: async (root, args, { req }, info) => {
      ...
      const patient = await Patient.findByIdAndUpdate(args.id, args.patientInput, { new: 'true' })
