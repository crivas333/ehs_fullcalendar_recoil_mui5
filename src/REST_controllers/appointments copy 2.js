...
exports.crudAppointments = async (req, res, next) => {
  console.log("crudAppointments: ", req.body)
  ...
  if (req.body.action === "insert" || (req.body.action === "batch" && req.body.added.length > 0)) {
      ...
      for (var a = 0; a < eventData.length; a++) {
          const appointmentId = await Counters.findOneAndUpdate({ _id: 'appointmentId'}, { $inc: { sequence_value: 1 } }, {new: true }).exec()
          ...
          try{
            const appointment = new Appointment({
              ...
              //eventData[a].EndTime = new Date(eventData[a].EndTime);
                ...
                patient:eventData[a].patient,
                fullName: fullName.fullName,
                comments: eventData[a].comments,
  ...
  if (req.body.action === "update" || (req.body.action === "batch" && req.body.changed.length > 0)) {
    ...
    for (var b = 0; b < eventData.length; b++) {
        ...
        try{
            ...
            const prevPatientRef = await  Appointment.findOne({appointmentId: eventData[b].appointmentId},'patient').exec()
            ...
            { //if patienRef has changed: delete appointmentRef from previous Patient
              if((eventData[b].patient !== prevPatientRef.patient)){
                const doc1 = await Patient.findOneAndUpdate({_id: prevPatientRef.patient}, 
                  {$pull: {'appointments': eventData[b]._id}}).exec()
          ...
          const appointment = await Appointment.findOneAndUpdate({appointmentId:eventData[b].appointmentId},
            {$set:{
              ...
              patient:eventData[b].patient,
              //fullName: currFullName.fullName||'',

              comments: eventData[b].comments,
              Subject: eventData[b].Subject,
              IsAllDay: eventData[b].IsAllDay,
            }},
            {new:true}).exec()
            
            await Patient.findOneAndUpdate({ _id: eventData[b].patient}, {$push: { appointments: appointment }}).exec()
  ...
  if (req.body.action === "remove" || (req.body.action === "batch" && req.body.deleted.length > 0)) 
  {
    ...
    for (var c = 0; c < eventData.length; c++) {
      ...
      const doc1 = await Patient.findOneAndUpdate({_id: eventData[c].patient}, 
  ...
  if (req.body.action === "remove111" || (req.body.action === "batch111" && req.body.deleted.length > 0)) 
  {
    (req.body.action === "remove") ? eventData.push({ Id: req.body.key }) : eventData = req.body.deleted;
    for (var c = 0; c < eventData.length; c++) {

      Patient.findOneAndUpdate({_id: eventData[c].patient}, 
        {$pull: {'appointments': eventData[c]._id}}, 
          function (err, model){ 
            //console.log('2deleted - eventData: ',req.body.deleted[0]._id)
            Appointment.findByIdAndRemove(req.body.deleted[0]._id, (err,appointment) => {
                if (err) res.json(err)
...
// @route   GET /api/v1/transactions
// @access  Public
exports.queryPatients = async (req, res, next) => {
  console.log("getPatients: ",req.body) 
  ...
  if (typeof req.body.where !== "undefined"){
    ...
    return res.status(200).json(await Patient.find({lastName: new RegExp(searchquery,'i')},'lastName lastName2 firstName historyId _id'))
